...
These results suggest that increasing model capacity primarily accelerates memorization rather than accelerating the discovery of the underlying algorithmic rule. Larger models quickly find high-capacity solutions that fit the training data, but the transition to a generalizable solution occurs on a comparatively stable timescale. The slightly earlier grokking observed for the smallest model may reflect limited memorization capacity, which forces the network to adopt a more structured solution sooner. For very large models, we observe a modest reduction in grokking time relative to intermediate widths, suggesting that strong regularization can still guide high-capacity networks toward simple solutions. However, this effect is weaker than the influence of model size on memorization.
